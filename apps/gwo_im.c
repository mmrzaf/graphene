...
static int cmp_desc_double_idx(const void *a, const void *b) {
    ...
    if ((int)da[1] < (int)db[1]) return -1;
    if ((int)da[1] > (int)db[1]) return 1;
    return 0;
}

// pick top-k candidate indices from position vector pos (size m)
// returns newly allocated array of size k (node indices are indices into candidates[])
static int* top_k_from_pos(const double *pos, int m, int k, const int *candidates) {
    ...
    if (!arr) fatal("malloc failed in top_k_from_pos");
    for (int i = 0; i < m; ++i) {
        arr[2*i + 0] = pos[i];
    ...
    if (!seeds) fatal("malloc failed in top_k_from_pos seeds");
    for (int i = 0; i < k; ++i) {
...
static double ic_spread(const Graph *g, const int *seeds, int k, double p, int runs) {
    ...
    for (int run = 0; run < runs; ++run) {
        ...
        if (!active) fatal("malloc failed in ic_spread active");
        int *queue = (int*)malloc(sizeof(int) * n);
        if (!queue) fatal("malloc failed in ic_spread queue");
        int qh = 0, qt = 0;
        ...
        int activated = 0;
        for (int i = 0; i < n; ++i) activated += active[i];
...
// Returns fitness value (higher better). Also returns sum_w via out_sum_w (if non-null).
// Implementation notes:
//  - Prob_receive approximated by combining direct contributions (p) and two-hop (p^2) per path.
...
static double compute_fitness(const Graph *g, const int *S, int k, double p, double *out_sum_w) {
    if (!g) return 0.0;
    int n = g->n;

    // mark seeds
    char *is_seed = (char*)calloc(n,1);
    if (!is_seed) fatal("malloc failed in compute_fitness is_seed");
    for (int i = 0; i < k; ++i) {
        if (S[i] >= 0 && S[i] < n) is_seed[S[i]] = 1;
    }

    // We'll accumulate contributions per node as product complement:
    // prob_receive = 1 - prod(1 - contrib) where contrib is p for direct edge, p^2 for 2-hop path
    double *one_minus = (double*)malloc(sizeof(double) * n);
    if (!one_minus) fatal("malloc failed in compute_fitness one_minus");
    for (int i = 0; i < n; ++i) one_minus[i] = 1.0;

    // For all seeds, add direct neighbors contributions and second-hop via neighbors
    for (int i = 0; i < k; ++i) {
        int s = S[i];
        if (s < 0 || s >= n) continue;
        int deg_s;
    ...
    if (!in_R) fatal("malloc failed in compute_fitness in_R");
    int Rcount = 0;
    for (int i = 0; i < k; ++i) {
        int s = S[i];
        if (s < 0 || s >= n) continue;
        if (!in_R[s]) { in_R[s] = 1; Rcount++; }
        int deg_s;
        const int *nbrs_s = graph_neighbors(g, s, &deg_s);
        for (int j = 0; j < deg_s; ++j) {
            int u = nbrs_s[j];
            if (!in_R[u]) { in_R[u] = 1; Rcount++; }
            // second neighbors
            int deg_u; const int *nbrs_u = graph_neighbors(g, u, &deg_u);
            for (int t = 0; t < deg_u; ++t) {
                int v = nbrs_u[t];
                if (!in_R[v]) { in_R[v] = 1; Rcount++; }
            }
        }
    }

    if (Rcount == 0) {
        free(is_seed); free(one_minus); free(in_R);
        if (out_sum_w) *out_sum_w = 0.0;
        return 0.0;
    }

    // compute worthiness W(u) = p_receive * deg(u)
    double *W = (double*)malloc(sizeof(double) * Rcount);
    if (!W) fatal("malloc failed in compute_fitness W");
    int idx = 0;
    double sumW = 0.0;
    for (int u = 0; u < n; ++u) {
        if (!in_R[u]) continue;
...
static int* build_candidates(const Graph *g, int *out_m) {
    ...
    if (!tmp) fatal("malloc failed in build_candidates tmp");
    int m = 0;
    for (int v = 0; v < n; ++v) {
    ...
    if (!cands) fatal("malloc failed in build_candidates cands");
...
int main(int argc, char **argv) {
    if (argc < 8) {
        fprintf(stderr,
                "Usage: %s <graph.edgelist> <k> <p> <pop_n> <max_iter> <seed> <ic_runs>\n"
    ...
    if (p <= 0.0 || p > 1.0) fatal("p must be in (0,1]");
    if (pop_n <= 0) pop_n = 50;
    if (max_iter <= 0) max_iter = 100;
    if (ic_runs <= 0) ic_runs = 50;
    ...
    if (m == 0) fatal("No eligible candidates (all nodes degree <= 1)");

    if (k > m) {
        fprintf(stderr, "Warning: k (%d) > candidate count (%d). Reducing k to %d\n", k, m, m);
        k = m;
    }

    // degrees of candidates (for seeding initialization)
    double *deg_c = (double*)malloc(sizeof(double) * m);
    if (!deg_c) fatal("malloc failed deg_c");
    for (int i = 0; i < m; ++i) deg_c[i] = (double)graph_degree(g, candidates[i]);
    ...
    if (!X || !fitness || !sumW) fatal("malloc failed population");

    for (int i = 0; i < pop_n; ++i) {
        X[i] = (double*)malloc(sizeof(double) * m);
        if (!X[i]) fatal("malloc failed X[i]");
    }
    ...
    for (int it = 0; it < max_iter; ++it) {
        ...
        for (int i = 0; i < pop_n; ++i) {
            if (i == alpha) continue;
            if (fitness[i] > fitness[beta]) beta = i;
        }
        for (int i = 0; i < pop_n; ++i) {
            if (i == alpha || i == beta) continue;
            if (fitness[i] > fitness[delta]) delta = i;
        }
        ...
        for (int w = 0; w < pop_n; ++w) {
            if (w == alpha) continue; // keep alpha
            // compute new Xw
            for (int j = 0; j < m; ++j) {
                ...
                if (newx < 0.0) newx = 0.0; // clip to non-negative
                X[w][j] = newx;
            }
        ...
        // optional regeneration: if beta or delta identical to alpha (by fitness), randomize them
        if (fitness[beta] == fitness[alpha]) {
            for (int j = 0; j < m; ++j) X[beta][j] = deg_c[j] * (0.5 + utils_rand_double());
        ...
        if (fitness[delta] == fitness[alpha] || fitness[delta] == fitness[beta]) {
            for (int j = 0; j < m; ++j) X[delta][j] = deg_c[j] * (0.5 + utils_rand_double());

...
static void metadata_free(void* data) {
    free(data);
}
...
static void print_neighbors(Graph* g, int node) {
    ...
    for (int i = 0; i < count; i++) printf("%d ", nbrs[i]);
    printf("\n");
}
...
int main(int argc, char** argv) {
    ...
    unsigned int flags = GRAPH_FLAG_UNDIRECTED |
                         GRAPH_FLAG_IGNORE_SELFLOOPS |
    ...
    printf("Graph loaded (undirected). Nodes=%d Edges=%d\n",
           graph_num_nodes(g2), graph_num_edges(g2));

...
void test_neighbors() {
    ...
    for (int i = 0; i < count; i++) {
        if (nbrs[i] == 1) found[0] = 1;
        if (nbrs[i] == 2) found[1] = 1;
        if (nbrs[i] == 3) found[2] = 1;
    }
...
static void bfs_visit(int node, int depth, void* ctx) {
    bfs_visit_count++;
}
...
static void dfs_visit(int node, void* ctx) {
    dfs_visit_count++;
}

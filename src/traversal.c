...
void graph_bfs(const Graph *g, int start, int max_depth, graph_visit_fn visit,
               void *ctx) {
  if (!g || start < 0 || start >= g->n || !visit)
    return;
...
void graph_dfs(const Graph *g, int start, graph_visit_simple_fn visit,
               void *ctx) {
  if (!g || start < 0 || start >= g->n || !visit)
    return;
